...
#Description: 2 step filtering without using the targets in B1 - C, filtered reads
...
abyss_dir=/extscratch/btl/kgagalova/trial_kollectorImproved/FilteredReadsProgressive/kollector.abyss
nam_bbt="PrototypeB0"
K=25
k=109
...
biobloommaker -i -k $K -p ${nam_bbt}1 -f 0.001 -t $j -n $n1 -s $rep_fil -r $r ${nam_bbt}.fa $READS_B1_R1 $READS_B1_R2

biobloomcategorizer -i -t $j -d ${nam_bbt}1 -f ${nam_bbt}1.bf -s $s $READS_B1_R1 $READS_B1_R2 >> ${nam_bbt}1.fastq
