...
#Description: A - Simple targets filter, all reads
...
base_dir=/extscratch/btl/kgagalova/trial_kollectorImproved/AllReadsSingleFilter
...
#i option here makes filtering less specific
biobloommaker -i -k $K -p $nam_bbt -f 0.001 -t $j -n $n0 -s $rep_fil $TARGETS 

biobloomcategorizer -i -t $j -d ${nam_bbt} -f ${nam_bbt}.bf -s $s <(zcat $r1) <(zcat $r2) >> ${nam_bbt}.fastq

mkdir -p $abyss_dir
cd $abyss_dir
abyss-pe -C $abyss_dir v=-v k=$k name=$nam_bbt np=$j lib='pet' pet=../${nam_bbt}.fastq long='longlib' longlib=$TARGETS B=18G H=4 kc=3
