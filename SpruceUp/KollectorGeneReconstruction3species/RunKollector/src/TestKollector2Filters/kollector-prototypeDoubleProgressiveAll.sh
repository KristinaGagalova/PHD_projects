...
#Description: 2 step filtering progressive - B, all reads 
...
base_dir=/extscratch/btl/kgagalova/trial_kollectorImproved/AllReadsDoubleFilterProgressive
...
biobloommaker -i -k $K -p ${nam_bbt}tar -f 0.001 -t $j -n $n1 -s $rep_fil -r $r ${nam_bbt}tar.fa <(zcat $r1) <(zcat $r2)

biobloomcategorizer -i -t $j -d ${nam_bbt}tar -f ${nam_bbt}tar.bf -s $s <(zcat $r1) <(zcat $r2) >> ${nam_bbt}tar.fastq
